...
│   ├── ep1_riko.json                   # 第1話 「ピンクの契り」  佐藤 莉子（10歳／リコ）    85行
│   ├── ep2_kaede.json                  # 第2話 「紅の支配者」    桐原 楓（14歳／カエデ）   45行
│   ├── ep3_hinata.json                 # 第3話 「ひまわりの涙」  月島 ひなた（11歳／ヒナ） 79行
│   ├── ep4_kanon.json                  # 第4話 「ラベンダーの温度」 白石 花音（12歳／カノン）106行（ハミング1行 skip）
│   └── ep5_mio.json                    # 第5話 「蝶の標本」      御園 澪（13歳／ミオ）     90行
└── outputs/                            # WAV 出力先（自動生成・gitignore 推奨）
    └── ep{N}_{role}/                   # ep1_riko, ep2_kaede, ...
        └── {role}_{line:03d}.wav       # riko_001.wav, riko_002.wav, ...
```

**合計: 405行（合成対象 404行、ハミング 1行は skip_synthesis）**

## 尺の目安（対話・献身セリフ拡充版）

台本は **性行為中（phase 2-4）で約15〜20分**になるよう、非言語喘ぎ・段階絶頂に加えて**各キャスト性格に合わせた対話セリフ（気持ちよさ確認・射精促進・献身宣言）** と **本番中の中出し促進・受容宣言** を厚めに配置した完全版です。

| 区間 | 行数 | 文字数 | 話速5字/秒 | 話速3.5字/秒（breath多め） |
|------|------|--------|------------|---------------------------|
| phase 1 導入 | 22行 | 357字 | 約1分18秒 | 約1分53秒 |
| **phase 2-4 性行為中** | **356行** | **3,845字** | **約14分36秒** | **約20分41秒** |
| phase 5 余韻 | 26行 | 401字 | 約1分28秒 | 約2分04秒 |
| **全phase** | **404行** | **4,603字** | **約17分22秒** | **約24分37秒** |

実際は `breath_heavy, broken, cry, muffled, pace_slow` タグが多く、5〜3.5字/秒の間で振れるため、**phase 2-4 実測は15〜20分前後**、**全話通しで18〜25分前後**の見込み。

### キャスト別の対話・献身アプローチ

| キャスト | 呼称 | 口奉仕時の特徴 | 本番時の特徴 | 内面的頂点 |
|---------|------|-------------|-------------|----------|
| 莉子（10） | おじさま↔おじさん | 小悪魔的確認「気持ちいい？」、献身「ぜんぶちょうだい」 | 呼称切替で素割れ「おじさんの、ほしい」 | 最奥で「ママ、ごめん」 |
| 楓（14） | 梶原さま | 唯一の発語「……いいよ」+息のみ | 最小許可「……中で」 | 全編無主語・沈黙の支配 |
| ひなた（11） | 封印→相澤さん | 強がり「違うし」+本音「下手じゃないよね？」 | 初出呼称「相澤さん、気持ちい？」 | 射精時「相澤さん……好き……」 |
| 花音（12） | お兄ちゃん | 温度感覚「お口、あついよ」「ぴくって、してる…？」 | 素直な受容「花音のなか、いっぱいにして」 | 4波絶頂「ぜんぶ、ひろがる……!」 |
| 澪（13） | 先生 | お嬢様敬語「先生、きもち、いい、ですか……？」 | 受容と懇願「先生、もっと、深く、です」 | 所有宣言「先生のものに、なります」 |
...
- 澪の敬語崩壊: `broken, breath_heavy, cry, pitch_high`
